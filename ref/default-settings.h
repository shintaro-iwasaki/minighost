...
param.percent_sum =
    100;                /* (Approximate) number of variables that to be summed.
...
param.stencil =
    21; /* Stencil selected. See MG_OPTIONS.F for parameter settings.   */
...
param.report_diffusion =
    20; /* Every param.report_diffusion time steps, report error.
...
/* Logical processor grid default (numpes, 1, 1) in main.c, but wanted to set
 * something here.  */
...
param.checkpoint_method =
    1; /* Checkpoint/restart implementation option.  See MG_OPTIONS.F  */
...
param.debug_grid =
    0; /* 1: zero domain, insert heat sourse (spike) into center.      */
