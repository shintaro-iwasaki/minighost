...
typedef struct {
    int scaling, nx, ny, nz, num_vars, percent_sum, num_spikes, num_tsteps,
        stencil, comm_method, bc, error_tol, report_diffusion, npx, npy, npz,
        report_perf, checkpoint_method, checkpoint_interval, restart_cp_num,
        debug_grid;

    char checkpoint_file[1024], restart_file[1024];
...
void MINI_GHOST(int *scaling, int *nx, int *ny, int *nz, int *num_vars,
                int *percent_sum, int *num_spikes, int *num_tsteps,
                int *stencil, int *comm_method, int *bc, int *error_tol,
                int *report_diffusion, int *npx, int *npy, int *npz,
                int *report_perf, int *checkpoint_method,
                int *checkpoint_interval, char *checkpoint_file,
                int *restart_cp_num, char *restart_file, int *debug_grid);
...
#include "default-settings.h"
    ...
    if (mype == 0) {
        for (i = 1; i < argc; i++) {
            if (!strcmp(argv[i], "--scaling"))
                ...
                if ((i + 1 == argc) || (strlen(argv[i + 1]) == 0)) {
                    fprintf(stderr, "The pathname of the checkpoint file is "
                                    "empty.  Aborting.\n");
                    print_help_message();
                }
                if (strlen(argv[i + 1]) > 1023) {
                    fprintf(stderr,
                            "The pathname of the checkpoint file is too long.  "
                            "It is limited to 1023 characters.  Aborting.\n");
                    print_help_message();
                }
                ...
                if ((i + 1 == argc) || (strlen(argv[i + 1]) == 0)) {
                    fprintf(stderr, "The pathname of the restart file is "
                                    "empty.  Aborting.\n");
                    print_help_message();
                }
                if (strlen(argv[i + 1]) > 1023) {
                    fprintf(stderr,
                            "The pathname of the restart file is too long.  It "
                            "is limited to 1023 characters.  Aborting.\n");
                    print_help_message();
                }
                ...
                print_help_message();
                MG_Assert(-1, "Listing of command line options requested to be "
                              "printed to stdio");
            } else {
                fprintf(stderr,
        ...
        switch (param.scaling) {
            ...
            default:
                ...
                fprintf(stderr,
                        "\n\n ** Error ** Unknown scaling %d; options are weak "
                        "(%d) and strong (%d). \n",
...
#if defined _MG_MPI
    ierr = MPI_Bcast(problem_params, count_problem_params, MPI_INT, root_pe,
                     MPI_COMM_WORLD);
    assert(ierr == MPI_SUCCESS);
    ierr = MPI_Bcast(param.checkpoint_file, 1024, MPI_CHAR, root_pe,
                     MPI_COMM_WORLD);
    assert(ierr == MPI_SUCCESS);
    ierr =
        MPI_Bcast(param.restart_file, 1024, MPI_CHAR, root_pe, MPI_COMM_WORLD);
    assert(ierr == MPI_SUCCESS);
#endif
    ...
    MINI_GHOST(&param.scaling, &param.nx, &param.ny, &param.nz, &param.num_vars,
               &param.percent_sum, &param.num_spikes, &param.num_tsteps,
               &param.stencil, &param.comm_method, &param.bc, &param.error_tol,
               &param.report_diffusion, &param.npx, &param.npy, &param.npz,
               &param.report_perf, &param.checkpoint_method,
               &param.checkpoint_interval, &(param.checkpoint_file[0]),
               &param.restart_cp_num, &(param.restart_file[0]),
               &param.debug_grid);
...
void print_help_message()
{
    ...
    fprintf(stderr,
            "--npdim : for cubes, i.e. npx=npy=npz. ( npdim^3 = numpes )\n");
#endif

    fprintf(stderr, " \n\n The following checkpoint options require "
    ...
    fprintf(stderr, "All associated settings are integers except "
                    "--checkpoint_file and --restart_file. \n");
...
int check_input(Input_params *param)
{
    ...
    if (param->stencil != 20 && param->stencil != 21 && param->stencil != 22 &&
        param->stencil != 23 && param->stencil != 24) {
        num_input_err++;
        fprintf(stderr,
                "[pe %d] ** Input error **: Unknown stencil option (%d). \n",
                mype, param->stencil);
    }
    if (param->num_vars <= 0) {
        num_input_err++;
    ...
    if (param->report_diffusion < 0) {
        num_input_err++;
        fprintf(stderr,
                "[pe %d] ** Input error **: report_diffusion %d < 0. \n", mype,
                param->report_diffusion);
    }
    if (param->percent_sum < 0 || param->percent_sum > 100) {
        num_input_err++;
        fprintf(stderr,
                "[pe %d] ** Input error **: percent_sum out of range: %d. \n",
                mype, param->percent_sum);
    }
    if (param->report_perf != 0 && param->report_perf != 1 &&
        param->report_perf != 2) {
        num_input_err++;
        fprintf(stderr,
                "[pe %d] ** Input error **: report_perf %d must be either "
                "(0,1,2). \n",
    ...
    if (param->debug_grid != 0 && param->debug_grid != 1) {
        num_input_err++;
        fprintf(stderr,
                "[pe %d] ** Input error **: debug_grid %d must be either "
                "(0,1). \n",
    ...
    if (param->npx * param->npy * param->npz != numpes) {
        num_input_err++;
        fprintf(stderr,
                "[pe %d] ** Input error **: npx*npy*npz not equal to numpes; "
                "(npx, npy, npz) = (%d, %d, %d).\n",
                mype, param->npx, param->npy, param->npz);
    }
    if (param->checkpoint_method != 1 && param->checkpoint_method != 2) {
        num_input_err++;
        fprintf(stderr,
                "[pe %d] ** Input error **: Unknown checkpoint method (%d). \n",
